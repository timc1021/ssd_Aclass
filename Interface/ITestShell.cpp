...
COMMAND_RESULT ITestShell::handleTestScript(const string& tcName)
{
	...
	if (tcName == "1_FullWriteAndReadCompare" || tcName == "1_") {
		script->run();
	}
	else if (tcName == "2_PartialLBAWrite" || tcName == "2_") {
		partialLBAWrite();
	}
	else if (tcName == "3_WriteReadAging" || tcName == "3_") {
		writeReadAging();
	}
	else if (tcName == "4_EraseAndWriteAging" || tcName == "4_") {
		eraseAndWriteAging();
	}
